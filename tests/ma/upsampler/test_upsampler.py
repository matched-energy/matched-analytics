...
def test_upsampler() -> None:
    ...
    trimmed_gen_by_supplier_data = gen_by_supplier_data.head(26)
        ...
        for year in result["timestamp"].dt.year.unique():
            for month in result["timestamp"].dt.month.unique():
                ...
                filtered_supplier = gen_by_supplier_data.pipe(
                    ...
                    f"and tech_category.str.lower() == @tech"
                )
    ...
    march_biomass_results = result[march_mask & (result["tech"] == "biomass")]["generation_mwh"]
