...
from ma.elexon.metering_data import metering_data_by_half_hour_and_bmu
from ma.elexon.S0142 import process_raw


def get_half_hourly_by_bmu() -> metering_data_by_half_hour_and_bmu.MeteringDataHalfHourlyByBmuType:
...
def test_rollup_bmus() -> None:
    ...
    half_hourly = metering_data_by_half_hour_and_bmu.rollup_bmus(
        metering_data_by_half_hour_and_bmu.segregate_import_exports(half_hourly_by_bmu)
    )
...
def test_filter_by_bmu_id() -> None:
    half_hourly_by_bmu = get_half_hourly_by_bmu()
    filtered_half_hourly_by_bmu = metering_data_by_half_hour_and_bmu.filter(half_hourly_by_bmu, bm_ids=["2__AGESL000"])
    assert len(filtered_half_hourly_by_bmu["bm_unit_id"].unique()) == 1


def test_plot() -> None:
    load = get_half_hourly_by_bmu()
    metering_data_by_half_hour_and_bmu.get_fig(load)
