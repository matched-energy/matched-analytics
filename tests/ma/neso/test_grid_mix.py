...
def test_groupby_tech_and_month() -> None:
    ...
    # Check that the grouping was done correctly, and all tech columns expst
    assert grouped.index.names == ["year", "month"]
    for tech in TechEnum:
        assert tech.value in grouped.columns

    expected_gas_sum = 538611.5  # Sum of gas column subset in Excel, divided by 2 to conver to MWH as load() does
